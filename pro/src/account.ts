...
export const checkProRunnableAndFixInplace = async (
  ...
  if (config.pro === undefined || config.pro.refreshToken === undefined) {
    if (config.conflictAction === "smart_conflict") {
      // allow smart conflict even without account
      return true;
    }
  ...
  for (const f of config.pro.enabledProFeatures) {
    const tooFarInTheFuture = f.expireAtTimeMs >= BigInt(Date.now() + msIn40Days);
    const alreadyExpired = f.expireAtTimeMs <= BigInt(Date.now());
  ...
  if (config.conflictAction === "smart_conflict") {
    // always allow smart conflict for testing
    /*
    if (
